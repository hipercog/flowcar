...
author: "Jussi Palomäki, Tuisku Tammi, Benjamin Ultan Cowley"
output: 
  html_document:
    toc: TRUE
    toc_float: TRUE
...
```{r combine_data pipeline chunk, include=FALSE}

### BACKRGOUND DATA ----
background17 <- read.csv("background_cogcarsim_2017.csv")
background19 <- read.csv("background_cogcarsim_2019.csv")

### Behavioral (CogCarSim) data ----
# import the cogcarsim2 databases
tb17 <- readCCSdb("cogcarsim2_2017.db", "run", 17)
tb19 <- readCCSdb("cogcarsim2_2019.db", "run", 19)
...
tbl_runs <- filter(tbl_runs, startsWith(participant, '0'))
...
### read Flow data ----
# fss17 <- read.csv("fss_data_2017.csv")
# fss19 <- read.csv("fss_data_2019.csv")
# fss <- rbind(fss17, fss19)
# fss <- fss %>% mutate(ID = seq(1:18))
fss <-
  data.table::fread("fss_data_2019.csv") %>%
  mutate(participant = participant + 9) %>%
  rbind(data.table::fread("fss_data_2017.csv"), .)
...
```{r analysis_pack, include=FALSE}
# attach packages
library(easypackages)
libraries("ggplot2", "gridBase", "colorRamps", "grDevices", "lme4", "effects",
          "multcomp", "lmerTest", "ggpubr", "MuMIn", "lattice",
...
# knitr::opts_knit$set(root.dir = normalizePath(file.path(here::here(), 'data')))

# create output folder?
odir <- file.path(here::here(), 'figures')
...
```{r H1 - replication, message=FALSE, warning=FALSE}
...
summary(H1b_lmer) # model summary
...
#ggsave("figure4.pdf", width=12, height=6)
```

# Hypothesis 2
```{r H2 - Effect of experience on the f~d effect, message=FALSE, warning=FALSE}
...
#Fit model using robust lmm to confirm result
...
#ggsave("combined_interaction_sliding.pdf", width=10, height=12)


```

# Other plots
```{r Other plots, message=FALSE, warning=FALSE}
...
#ggsave("partial_effects_lmer.pdf", width=6, height=5)
...
#ggsave("lagged_flow.pdf", width=7, height=6)
...
############
#Custom Cronbach's alpha calculator for each trial. 1= full scale, 2 = fluency, 3 = absorption
############
